...
interface MonitorAttendanceProps {
    search: string
    startDate: string
    endDate: string
}

export const MonitorAttendance = ({ search, startDate, endDate }: MonitorAttendanceProps) => {
                            ...
                            {isFetching ? (
                                ...
                                <tr>
                                    <td colSpan={7} className="py-10 text-center text-sm text-red-500">
                                        {error.message == "User not found" ? "User not found" : "Failed to load attendance records."}
                                    </td>
                                </tr>
                                ...
                                currentData.map((item: ListAttendance) => {
                                    return (
                                        ...
                                        >
                                            <td className="p-4 text-sm text-zinc-700">{formatDate(item.workday.date).split(",")[0]}</td>
                                            <td className="p-4 text-sm text-zinc-700">{item.user.fullName}</td>
                                            <td className="p-4 text-sm text-zinc-700">{item.user.department}</td>
                                            <td className="p-4 text-sm text-zinc-700">{formatDate(item.checkInAt)}</td>
                                            <td className="p-4 text-sm text-zinc-700">{formatDate(item.checkOutAt)}</td>
                                            <td className="p-4 text-sm text-zinc-700">{item.notes}</td>
                                            <td className="p-4">
                                                <Button
                                                    size="sm"
                                                    variant="outline"
